...
// ---------- Фоновая музыка ----------
// Лёгкая мажорная колыбельная-вальс: тихое арпеджио + редкая мелодия
let musicSource = null, musicOn = false;
function makeMusic() {
    // zzfxM: инструменты, паттерны, секвенция, BPM
    const inst = [
        [ .25, 0, 140, .1, .3, .5, 0, 1.5, 0, 0, 0, 0, 0, 0, 0, 0, 0, .6, .3 ],   // мягкий бас-щипок
        [ .15, 0, 560, .05, .2, .4, 0, 1.2, 0, 0, 0, 0, 0, 0, 0, 0, 0, .5, .2 ],  // колокольчик-мелодия
    ];
    // до-мажор, вальсовый рисунок
    const P = (b, m) => [ b, m ];
    const pat = [
        [ // паттерн 0
            [ 1, 0, 13, 0, 0, 17, 0, 0, 20, 0, 0, 17, 0, 0, 13, 0, 0, 9, 0, 0, 13, 0, 0, 17, 0 ],
            [ 2, 0, 0, 0, 25, 0, 0, 0, 0, 27, 0, 0, 0, 0, 29, 0, 0, 0, 0, 25, 0, 0, 0, 0, 0 ],
        ],
        [ // паттерн 1
            [ 1, 0, 11, 0, 0, 15, 0, 0, 18, 0, 0, 15, 0, 0, 11, 0, 0, 8, 0, 0, 11, 0, 0, 15, 0 ],
            [ 2, 0, 0, 0, 23, 0, 0, 0, 0, 25, 0, 0, 0, 0, 27, 0, 0, 0, 0, 30, 0, 0, 0, 0, 0 ],
        ],
    ];
    return zzfxM(inst, pat, [0, 0, 1, 0], 72);
}
function startMusic() {
    if (musicOn || (S && S.mute)) return;
    try {
        const buf = makeMusic();
        musicSource = playSamples(buf, .5, 1, 0, true);
        musicOn = true;
    } catch(e) {}
}
function stopMusic() {
