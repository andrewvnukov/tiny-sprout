...
// Цикл из 7 дней, пропущенный день обнуляет серию. Монеты привязаны к доходу
// игрока, иначе к поздней игре награда обесценивается; семена — на 3-й и 7-й день.
const STREAK_DAYS = 7;
const STREAK_SEEDS = [0, 0, 1, 0, 0, 0, 2];              // индекс = день цикла (0..6)
const streakCoins = i => Math.max(300, Math.round(S.bestIps * 60 * (2 + i)));

// ---------- Офлайн ----------
const OFFLINE_CAP    = 12*3600;   // сколько максимум «доигрывается» офлайн, сек
